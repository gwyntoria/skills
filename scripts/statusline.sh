#!/bin/bash
# Claude Code statusline: Model effort | cwd on branch | Context % left | tokens used
...
if [ "$remaining_pct" -le 15 ] 2>/dev/null; then
  remaining_color="$RED"
...
# 4. Total tokens used
total_tokens="${used_tokens:-0}"
if [ "$total_tokens" -ge 1000000 ] 2>/dev/null; then
  tokens_display=$(awk -v t="$total_tokens" 'BEGIN { printf "%.1fM", t/1000000 }')
